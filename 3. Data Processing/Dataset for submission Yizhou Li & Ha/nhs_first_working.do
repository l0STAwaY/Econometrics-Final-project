...
use "/Users/apple/Documents/GitHub/Econometrics-Final-project/3. Data Processing/Dataset for submission/merged_data_description.dta"
...
//re-order which are correctly that is low(1)->medium(2)->high(3) and incorrectly ordered  health(F) dvint(F) poverty(T) earnings(T) wormedbill(F) educ(T)


// 500 is never
tabulate dvint
tabulate  impute_dvint
replace impute_dvint = 500 if dvint == 100

tabulate health
tabulate impute_health //1 = Poor, 2 = Fair, 3 = Good, 4 = Very Good, 5 = Excellent
replace impute_health  = 1 if health == 5
replace impute_health  = 2 if health == 4
replace impute_health  = 4 if health == 2
replace impute_health  = 5 if health == 1

tabulate wormedbill 
tabulate impute_wormedbill //1 = Not at all worried  2=Somewhat worried 3= very worried



replace impute_wormedbill = 1 if wormedbill == 3
replace impute_wormedbill = 3 if wormedbill == 1
...
esttab reg_missing_earnings reg_missing_health reg_missing_dvint reg_missing_poverty reg_missing_himedicaidyr reg_missing_wormedbill using missingness_test.rtf, replace eform stat(r2_p se p)
...
mi estimate, post: ologit impute_dvint impute_earnings, robust
est store model1


mi estimate,post: ologit impute_dvint impute_earnings impute_health, robust
est store model2


mi estimate, post: ologit impute_dvint impute_earnings impute_health age, robust
est store model3


mi estimate, post: ologit impute_dvint impute_earnings impute_health age sex, robust
est store model4


mi estimate, post: ologit impute_dvint impute_earnings impute_health age sex racenew, robust
est store model5

mi estimate, post: ologit impute_dvint impute_earnings impute_health age sex impute_mcareprob racenew age_dummy impute_mcareprob_dummy, robust
est store model6 

esttab model1 model2 model3 model4 model5 model6 using mi_ologit_dummy_table.rtf, replace

eststo clear


* First model with condition age < 65 new prf
mi estimate, post: ologit impute_dvint impute_earnings if age < 65, robust
est store model1


mi estimate, post: ologit impute_dvint impute_earnings impute_health if age < 65, robust
est store model2


mi estimate, post: ologit impute_dvint impute_earnings impute_health age if age < 65, robust
est store model3


mi estimate, post: ologit impute_dvint impute_earnings impute_health age sex if age < 65, robust
est store model4


mi estimate, post: ologit impute_dvint impute_earnings impute_health age sex racenew if age < 65, robust
est store model5

esttab model1 model2 model3 model4 model5 using mi_ologit_nodummy_table.rtf, replace
...
mimrgns, dydx(impute_earnings) atmeans predict(outcome(500))
...
foreach outcome in 203 204 302 305 400 500{
...
margins, dydx(impute_earnings) post
eststo marginal_fei

esttab marginal_fei using "marginal_fixed_effect_income.rtf", replace label title("Marginal and Fixed Effect of Income on Dvint")
...
eststo clear
drop ft_xb
regress impute_earnings impute_himedicaidyr i.year impute_health age sex racenew if age < 65 , robust  //lin prob
predict ft_xb, xb
ologit impute_dvint ft_xb impute_health age sex racenew i.year if age < 65 
margins, dydx(ft_xb) post
estimates store m1, title(Model 1)




count if impute_dvint == 305 | impute_dvint ==  400 | impute_dvint == 500 & sex==1
count if impute_dvint == 305 | impute_dvint ==  400 | impute_dvint == 500 & sex==2

count if impute_dvint== 203 & sex ==1 
count if impute_dvint== 203 & sex ==2 


count if impute_dvint== 203 | impute_dvint== 302 & sex ==1 
count if impute_dvint== 203  |impute_dvint== 302 & sex ==2 
...
regress impute_wormedbill impute_himedicaidyr i.year impute_health age sex racenew age if age < 65 , robust 
predict fw_xb, xb
ologit impute_dvint fw_xb impute_health age sex racenew i.year if age < 65 
margins, dydx(fw_xb) post
estimates store m2, title(Model 2)

esttab m1 m2 using margins_results.rtf, replace label title("Margins Results")
